...
DNS has strict rules to ensure compatibility across all global systems:
...
The IP is assigned for a limited time. After expiration, the client must renew/re-run discovery. The client may also release the IP when disconnecting.
...
### Technical Note: Network Address Translation (NAT)
...
**DORA Process:** The 4-step handshake used to get an IP:  
